...
final readonly class PostgresReplayVerifier implements MigrationReplayVerifier
{
    ...
    public function verify(
        ...
        $sandboxes = new PostgresReplaySandboxManager($this->databases, $this->sourceConnection);
        $sourceSandbox = null;
        ...
        try {
            ...
            $source = $this->runner()->replay(
                ...
                $migrationTable,
                [$migrationTable, PostgresReplaySandboxManager::MARKER_TABLE],
                $this->inspector,
                'source',
            );
            ...
            if ($this->databases->getDefaultConnection() !== $defaultConnection) {
                ...
                $cleanupFailure ??= ReplayVerificationFailed::because(
                    'default database connection changed during PostgreSQL replay.',
                );
            }

            if ($cleanupFailure !== null) {
                throw ReplayVerificationFailed::because(
                    'PostgreSQL replay cleanup did not complete safely.',
                    $cleanupFailure,
                );
            }
        }
    ...
    private function assertMigrationTable(string $migrationTable): void
    {
        if (preg_match('/\A[A-Za-z_][A-Za-z0-9_]*\z/', $migrationTable) !== 1) {
            throw ReplayVerificationFailed::because(
                "migration repository table [{$migrationTable}] is invalid for PostgreSQL replay.",
            );
        }

        if (strcasecmp($migrationTable, PostgresReplaySandboxManager::MARKER_TABLE) === 0) {
            throw ReplayVerificationFailed::because(
    ...
    private function cleanupSandboxes(
        PostgresReplaySandboxManager $manager,
        ?PostgresReplaySandbox $baseline,
        ?PostgresReplaySandbox $source,
    ): ?Throwable {
        $failure = null;
